...
public class TimedTaskJobHandler {
    ...
    @Autowired(required = false)
    private List<QuotesEvent> quotesEvents;
    ...
    @XxlJob("everyMinuteQuotes")
    public ReturnT<String> everyMinuteQuotes(String param)  {
        log.info("get  quotes");

        for (int i = 0; i < quotesEvents.size(); i++) {
            try {
                quotesEvents.get(i).syncQuotesData();
            } catch (Exception e) {
                log.error("每分钟任务异常", e);
            }
        }
