...
//        String phone = cache.get(CachePrefix.FIND_MOBILE + uuid).toString();
        ...
        if (username != null) {
            //修改密码
            LambdaUpdateWrapper<Member> lambdaUpdateWrapper = Wrappers.lambdaUpdate();
            lambdaUpdateWrapper.eq(Member::getMobile, username);
            lambdaUpdateWrapper.set(Member::getPassword, new BCryptPasswordEncoder().encode(password));
    ...
    @PointLogPoint
    @Transactional(rollbackFor = Exception.class)
    public Boolean updateMemberPointClass(Long point, String type, String memberId, String content, PointEnum pointType) {
        //获取当前会员信息
        Member member = this.getById(memberId);
        ...
        if (member != null) {
            ...
            switch (pointType){
                case POINT_A:
                    member.setPointA(member.getPointA()+varpoint);
                    break;
                case POINT_B:
                    member.setPointB(member.getPointB()+varpoint);
                    break;
                case POINT_C:
                    member.setPointC(member.getPointC()+varpoint);
                    break;
