...
import cn.lili.rocketmq.tags.PointEnum;
...
public class MemberPointsHistoryServiceImpl extends ServiceImpl<MemberPointsHistoryMapper, MemberPointsHistory> implements MemberPointsHistoryService {
    ...
    @Override
    public MemberPointsHistoryVO getMemberPointsHistoryVO(String memberId,String pointClass) {
        //获取会员积分历史
        Member member = memberService.getById(memberId);
        MemberPointsHistoryVO memberPointsHistoryVO = new MemberPointsHistoryVO();
        PointEnum pointEnum = PointEnum.valueOf(pointClass);
        if (member != null) {
        switch (pointEnum){
            case   POINT_A:
                memberPointsHistoryVO.setPoint(member.getPointA());
                memberPointsHistoryVO.setTotalPoint(member.getPointA());
                break;
            case  POINT_B:
                memberPointsHistoryVO.setPoint(member.getPointB());
                memberPointsHistoryVO.setTotalPoint(member.getPointB());
                break;
            case POINT_C:
                memberPointsHistoryVO.setPoint(member.getPointC());
                memberPointsHistoryVO.setTotalPoint(member.getPointC());
                break;
            default:break;
        }
